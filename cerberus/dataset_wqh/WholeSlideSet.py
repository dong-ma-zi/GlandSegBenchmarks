...
def ColorDeconv(img: np.uint8):
    ...
    return Image.fromarray(np.uint8(img_h * 255))
...
class WholeSlideSet(Dataset):
    ...
    def get_roi_region(self, mask_level, mthresh=7, use_otsu=False, sthresh=20, sthresh_up=255, close=0):
        img = np.array(self.slide.read_region((0, 0), mask_level, self.slide.level_dimensions[mask_level]).convert('RGB'))
