...
class PatchExtractor(object):
    ...
    def __get_patch(self, x, ptx):
        ...
        # debug, 当patch尺寸不足时padding
        if win.shape[0] != self.win_size[0] or win.shape[1] != self.win_size[1]:
            padding = np.zeros((self.win_size[0], self.win_size[1], 6))
            padding[:win.shape[0], :win.shape[1]] = win
            win = padding
