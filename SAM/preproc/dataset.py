...
class __CoNSeP(__AbstractDataset):
    ...
    def load_ann(self, path, with_type=False):
        ...
        if with_type:
            ...
            # merge classes for CoNSeP (in paper we only utilise 4 nuclei classes and background)
            # If own dataset is used, then the below may need to be modified
            # ann_type[(ann_type == 3) | (ann_type == 4)] = 3
            # other, inflammatory, healthy epithelial, dysplastic/malignant epithelial, spindle-shaped
            # ann_type[(ann_type == 5) | (ann_type == 6) | (ann_type == 7)] = 5
