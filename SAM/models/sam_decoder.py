...
class MaskDecoder(nn.Module):
    def __init__(
        ...
        *,
        ...
        if pretrained_path:
            self.load_pretrained_weight(torch.load(pretrained_path))
    ...
    def load_pretrained_weight(self, pretrained_dict):
        load_weight = {k.replace('mask_decoder.', ''): v for k, v in pretrained_dict.items()
                       if k.replace('mask_decoder.', '') in self.state_dict() and
                       self.state_dict()[k.replace('mask_decoder.', '')].numel() == v.numel()}
        self.load_state_dict(load_weight, strict=False)
