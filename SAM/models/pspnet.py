...
from .psp_decoder import PSPDecoder
...
class PSPNet(SegmentationModel):
    ...
    *encoder* and *Spatial Pyramid* (decoder). Spatial Pyramid build on top of encoder and does not
    ...
    def __init__(
        ...
        self.encoder = WideRes38()
        self.encoder.load_state_dict(torch.load("/home/data1/my/Project/GlandSegBenchmark/OEEM/classification/weights/res38d.pth"),
                                     strict=False)
        ...
        self.decoder = PSPDecoder(
            ...
            encoder_channels=[256, 512, 1024, 4096],
...
x = torch.ones(size=(16, 3, 512, 512))
y = PSPNet(classes=2)
z = y(x)
u = 1