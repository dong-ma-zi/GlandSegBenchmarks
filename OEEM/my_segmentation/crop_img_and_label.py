...
# gtd = sys.argv[2]
# outd = sys.argv[3]
imgd = '/home/data1/my/Project/GlandSegBenchmark/OEEM/classification/glas_cls/1.training/origin_ims/'
gtd = '/home/data1/my/Project/GlandSegBenchmark/OEEM/classification/glas_seg/WRes38_best_train_pseudo_mask/'
outd = 'glas_seg_up0915'
...
for f in os.listdir(imgd):
    ...
    for h_i in range(h_n):
        for w_i in range(w_n):
            ...
            out_name = f.replace(img_type, '_{:d}_{:d}_{:d}_{:d}_{:d}_{:d}.png'.format(h, w, y1, y2, x1, x2))
            cv2.imwrite(os.path.join(out_imgd, out_name), crop_img)
            crop_gt = gt[y1:y2, x1:x2]
            cv2.imwrite(os.path.join(out_gtd, out_name.replace('.png', gt_type)), crop_gt)
