...
def prepare_glas(side_length: int, stride: int, scales: List[int], network_image_size: int) -> None:
    ...
    image_path = 'glas_cls/1.training/origin_ims'
    # mask_path = 'glas_cls/1.training/mask'
    mask_path = '/home/data2/MedImg/GlandSeg/GlaS/Warwick_QU_Dataset/'

    destination = f'glas_cls/1.training/img_{side_length}_{stride}/' # the output directory of the cropped images
    ...
    random.seed(2023)
    random.shuffle(training_ims)
    train_set = training_ims[:int(0.8 * len(training_ims))]
    valid_set = training_ims[int(0.8 * len(training_ims)):]

    split_dict = {'Train': train_set,
                  'Valid': valid_set}
    summary = []

    print(f'start generating train images ...')

    for set_name, data_set in split_dict.items():
        os.makedirs(os.path.join(destination, set_name))
        os.makedirs(os.path.join(destination, set_name, 'Images'))
        os.makedirs(os.path.join(destination, set_name, 'Labels'))

        # for i in tqdm(range(1, len(training_ims)+1)):
        for i in tqdm(range(1, len(data_set) + 1)):
            ...
            for j in range(len(crop_list)):
                ...
                Image.fromarray(crop_im).save(os.path.join(destination, set_name,
                                                           'Images',
                                                           f'{set_name}_{i}_{j}-[{has_tumor}, {has_normal}].png'))
                Image.fromarray(crop_mask).save(os.path.join(destination, set_name,
                                                             'Labels',
                                                             f'{set_name}_{i}_{j}-[{has_tumor}, {has_normal}].png'))
