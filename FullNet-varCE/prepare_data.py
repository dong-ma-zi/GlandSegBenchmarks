...
def main():
    data_set = 'GlaS'
    dset = 'TrainSet'
    original_data_dir = f'/home/data2/MedImg/GlandSeg/{data_set}/train/250'

    img_dir = f'/home/data2/MedImg/GlandSeg/{data_set}/train/{dset}/Images/'
    label_instance_dir = f'/home/data2/MedImg/GlandSeg/{data_set}/train/{dset}/Annotation/'
    label_dir = f'/home/data2/MedImg/GlandSeg/{data_set}/train/250/{dset}_tmp'
    weightmap_dir = f'/home/data2/MedImg/GlandSeg/{data_set}/train/{dset}/weight_maps/'
    patch_folder = '{:s}/patches'.format(original_data_dir)

    create_folder(weightmap_dir)
    ...
    split_patches(img_dir, '{:s}/images/train'.format(patch_folder))
    split_patches(label_dir, '{:s}/labels/train'.format(patch_folder), 'label')
    split_patches(weightmap_dir, '{:s}/weight_maps/train'.format(patch_folder), 'weight')
...
def create_ternary_labels(data_dir, save_dir):
    ...
    for image_name in sorted(image_list):
        ...
        # extract edges
        id_max = np.max(image)
        contours = np.zeros((h, w), dtype=bool)
        nuclei_inside = np.zeros((h, w), dtype=bool)
