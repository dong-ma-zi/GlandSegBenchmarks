...
from utils import gland_accuracy_object_level_all_images, gland_accuracy_object_level, \
...
def validate(model, save_dir):
    ...
    for batch_idx, image_filename in enumerate(images_list):
        ...
        #if image_filename in ['testB_11.bmp', 'testB_19.bmp','testB_18.bmp', 'testB_14.bmp']:
        #    continue
        ...
        ## 如果用了自己定义的dataloader,训练过程采用Image.open加载图像，与测试通道相反，需要做通道变换
...
def extract_prompt_and_save(pred_labeled, name):
    props = measure.regionprops(pred_labeled)
    points = []
    bboxes = []
    for prop in props:
        point = prop.centroid
    ...
    #np.save('{:s}/{:s}.npy'.format(args.prompt_save_dir, name), bboxes)
    scipy.io.savemat('{:s}/{:s}.mat'.format(args.prompt_save_dir, name), {'points':points, 'boxes':bboxes})
