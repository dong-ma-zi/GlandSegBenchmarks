...
def crop_validation_images(dataset_path, side_length, stride, scales, validation_folder_name, ck='_320_256'):
    """
    ...
    if not os.path.exists(f'{validation_folder_name}/crop_images{ck}'):
        os.mkdir(f'{validation_folder_name}/crop_images{ck}')
    
    for image in tqdm(images):
        if not os.path.exists(f'{validation_folder_name}/crop_images{ck}/{image.split(".")[0]}'):
            os.mkdir(f'{validation_folder_name}/crop_images{ck}/{image.split(".")[0]}')
        ...
        for i in range(len(scales)):
            ...
            if not os.path.exists(f'{validation_folder_name}/crop_images{ck}/{image.split(".")[0]}/{scales[i]}'):
                os.mkdir(f'{validation_folder_name}/crop_images{ck}/{image.split(".")[0]}/{scales[i]}')
            
            for j in range(len(scaled_im_list[i])):
                scaled_im_list[i][j].save(f'{validation_folder_name}/crop_images{ck}/{image.split(".")[0]}/{scales[i]}/{scaled_position_list[i][j]}.png')
